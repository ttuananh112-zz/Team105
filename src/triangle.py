...
def centroid(layer, drop_rate= 0.45, shrink_rate= 0.2):
    """
    ...
    error = float('inf')
    point1= np.array([layer.shape[0]-1,0], dtype=np.float) #left
    point2= np.array([layer.shape[0]-1,layer.shape[1]-1], dtype=np.float) #right
    result= layer.shape[1]/2

    for i in range(layer.shape[1] /16, layer.shape[1], layer.shape[1] /8):
        point3= np.array([top, i])
        err= cal_error(cv2.resize(layer, (0,0), fx=shrink_rate, fy=shrink_rate), point1 * shrink_rate, point2 * shrink_rate, point3 * shrink_rate)
        if (err<error):
            error = err
            result= i
    ...
    # # print cen
    # point1= (0, layer.shape[0]-1)
    # point2= (layer.shape[1]-1, layer.shape[0]-1)
    # point3= (result,int(top))
    # cv2.line(layer, point1, point3, (255,0,0),3)
    # cv2.line(layer, point2, point3, (0,255,0),3)
    # cv2.circle(layer,(int(cen[1]),int(cen[0])), 5, (0,255,0), -1,)
    # cv2.imshow('.', layer)
    # cv2.waitKey(0)
    return np.array([top, result])
